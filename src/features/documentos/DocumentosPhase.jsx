...
function DocumentosPhase({ cfg, issues, cipData, docData, docTab, setDocTab, setVulnF, stats, sonarData, dl1, dlAll, showSources, setShowSources, getSourcesDisplay, TODAY, card, inp, ta, infoBox, warnBox, lbl, btnS, btnG, sevBadge, fetchAI, fetchAI_DT, fetchAI_field, aiLoading, claudeMcpStatus, repos, darkMode }) {
  return (
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,paddingBottom:18,borderBottom:"1px solid var(--border)"}}>
                <div>
                  <div style={{fontSize:10,color:"var(--accent)",letterSpacing:2,fontFamily:"monospace",marginBottom:4}}>FASE 2 — DOCUMENTACIÓN</div>
                  <div style={{fontSize:24,fontWeight:800,color:"var(--text-bright)"}}>Generación de los 4 Documentos</div>
                  <div style={{fontSize:12,color:"var(--text-secondary)",marginTop:4}}>Auto-llenados con datos del reporte. Edita y descarga.</div>
                </div>
              ...
              <div style={infoBox}><strong style={{color:"var(--accent)"}}>Auto-llenado:</strong> Archivos, tipos de vulnerabilidad, repos y estimaciones se extraen directamente de tu Excel.</div>
              ...
              <div style={card({padding:0,overflow:"hidden"})}>
                <div style={{display:"flex",borderBottom:"1px solid var(--border)",background:"var(--bg-base)"}}>
                  {["📄 Diseño General","📐 Diseño Técnico","✅ Checklist","📦 CIP"].map((t,i)=>(
                    <button key={i} style={{padding:"10px 18px",border:"none",background:docTab===i?"var(--bg-card)":"transparent",color:docTab===i?"var(--accent)":"var(--text-muted)",cursor:"pointer",fontSize:12,fontWeight:600,borderBottom:docTab===i?"2px solid var(--accent)":"2px solid transparent",transition:"all 0.15s"}} onClick={()=>setDocTab(i)}>{t}</button>
                  ))}
                </div>
                <div style={{padding:20}}>
                  ...
                  {docTab===0&&(
                    <div>
                      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
                        <button
                          style={{...btnG, color:"var(--accent)", borderColor:"#1A3A5C"}}
                      ...
                      {/* App description */}
                      <div style={card({background:"var(--bg-base)",marginBottom:16})}>
                        <div style={{fontSize:11,fontWeight:700,color:"var(--accent)",letterSpacing:1.5,marginBottom:10,textTransform:"uppercase",fontFamily:"monospace"}}>🏗 Diseño de la Aplicación</div>
                      ...
                      {stats.byType.map(({type,count})=>{
                        ...
                        return (
                          <div key={type} style={card({background:"var(--bg-base)",border:"1px solid #1E3050"})}>
                            {/* Header */}
                            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                              <div style={{display:"flex",alignItems:"center",gap:10}}>
                                <span style={{fontSize:22}}>{kb.icon}</span>
                                <div>
                                  <div style={{fontWeight:700,color:"var(--text-bright)",fontSize:13}}>{kb.label !== type ? kb.label : type}</div>
                                  <div style={{fontSize:10,color:"var(--text-muted)",fontFamily:"monospace",marginTop:2}}>{count} issues · {typeFiles.length} archivos únicos · {typeHrs} hrs</div>
                                </div>
                              </div>
                              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                                ...
                                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                                  <div style={{display:"flex",gap:6}}>
                                    ...
                                    >↺ Restaurar KB</button>
                                    <button
                                      style={{...btnG,fontSize:10,padding:"4px 10px",color:showSources[type]?"var(--accent)":"var(--text-secondary)",borderColor:showSources[type]?"var(--border-mid)":"var(--border)"}}
                            ...
                            {showSources[type]&&(
                              <div style={{marginBottom:12,padding:"10px 14px",background:"#00D4FF08",border:"1px solid #00D4FF20",borderRadius:7}}>
                                <div style={{fontSize:9,color:"var(--text-muted)",fontFamily:"monospace",letterSpacing:1.5,marginBottom:7}}>FUENTES OFICIALES CONSULTADAS PARA ESTA BASE DE CONOCIMIENTO</div>
                                <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                                  {getSourcesDisplay(type).map((s,i)=>(
                                    <a key={i} href={s.url} target="_blank" rel="noopener noreferrer"
                                      style={{fontSize:10,color:"var(--accent)",background:"#00D4FF10",padding:"3px 9px",borderRadius:4,border:"1px solid #00D4FF30",textDecoration:"none",fontFamily:"monospace"}}>
                                      🔗 {s.label}
                                    </a>
                                  ))}
                                </div>
                              ...
                              {[
                                ...
                                return (
                                  <div key={key}>
                                    ...
                                    <textarea
                                      style={{...ta,minHeight:130,fontSize:11,borderColor:accent?"var(--border-mid)":"var(--border)",color:"var(--text-primary)"}}
                                      value={ov[key]||""}
                            ...
                            {/* Files */}
                            <div style={{marginTop:10,padding:"8px 12px",background:"var(--bg-panel)",borderRadius:6,border:"1px solid var(--border)"}}>
                              <div style={{fontSize:9,color:"var(--text-dim)",fontFamily:"monospace",marginBottom:5}}>ARCHIVOS IMPACTADOS ({typeFiles.length})</div>
                              <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
                                {typeFiles.slice(0,8).map((fp,i)=>(
                                  <span key={i} style={{fontSize:10,fontFamily:"monospace",color:"var(--accent)",background:"var(--bg-base)",padding:"2px 6px",borderRadius:4,border:"1px solid var(--border)"}}>{fname(fp)}</span>
                                ))}
                                {typeFiles.length>8&&<span style={{fontSize:10,color:"var(--text-muted)"}}>+{typeFiles.length-8} más</span>}
                              </div>
                            </div>
                          </div>
                        );
                  ...
                  {docTab===1&&(
                    <div>
                      ...
                      {stats.byType.map(({type,count})=>{
                        ...
                        const rowS={display:"grid",gridTemplateColumns:"180px 1fr",borderBottom:"1px solid var(--border)"};
                        const lblS={padding:"10px 14px",background:"var(--bg-input)",color:"var(--text-muted)",fontSize:11,fontWeight:700,display:"flex",alignItems:"flex-start",paddingTop:12};
                        const valS={padding:"8px 12px",background:"var(--bg-base)"};
                        return (
                          <div key={type} style={{marginBottom:24,border:"1px solid #1E3050",borderRadius:8,overflow:"hidden"}}>
                            {/* Card header */}
                            <div style={{padding:"10px 16px",background:"var(--bg-card)",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid var(--border)"}}>
                              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                                <span style={{fontSize:20}}>{kb.icon}</span>
                                <div>
                                  <div style={{fontWeight:700,color:"var(--text-bright)",fontSize:13}}>{kb.label!==type?kb.label:type}</div>
                                  <div style={{fontSize:10,color:"var(--text-muted)",fontFamily:"monospace"}}>{count} issues · {typeFiles.length} archivos</div>
                                </div>
                              </div>
                            ...
                            <div>
                              ...
                              <div style={rowS}>
                                ...
                                <div style={{...valS,display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                                  ...
                                  <span style={{color:"var(--text-muted)",fontSize:11,whiteSpace:"nowrap"}}>Alineación:</span>
                              ...
                              {/* Vuln name row */}
                              <div style={rowS}>
                                <div style={{...lblS,color:"var(--accent)"}}>{kb.label!==type?kb.label:type}</div>
                              ...
                              <div style={{...rowS,borderBottom:"none"}}>
                                ...
                                <div style={valS}>
                                  ...
                                  {repoUrls.length>0&&(
                                    <div style={{marginTop:8,padding:"8px 10px",background:"var(--bg-panel)",borderRadius:6,border:"1px solid var(--border)"}}>
                                      <div style={{fontSize:9,color:"var(--text-muted)",fontFamily:"monospace",marginBottom:5,letterSpacing:1}}>REPOSITORIOS</div>
                                      {repoUrls.map((u,i)=>(
                                        <div key={i}><a href={u} target="_blank" rel="noopener noreferrer" style={{fontSize:11,color:"var(--accent)",fontFamily:"monospace",wordBreak:"break-all"}}>{u}</a></div>
                                      ))}
                                    </div>
                                  )}
                                </div>
                  ...
                  {docTab===3&&(
                    <div>
                      ...
                      <div style={infoBox}><strong style={{color:"var(--accent)"}}>CIP auto-generado:</strong> {cipData.length} archivos únicos agrupados de {stats.total} issues. Columnas: archivo, tipo, acción, severidad máxima, conteo, horas.</div>
                      <div style={{overflowX:"auto"}}>
                        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                          <thead><tr>{["#","Archivo","Tipo","Acción","Issues","Hrs","Sev."].map(h=><th key={h} style={{padding:"7px 10px",borderBottom:"1px solid var(--border)",textAlign:"left",fontSize:10,color:"var(--text-muted)",letterSpacing:1,textTransform:"uppercase",fontFamily:"monospace",background:"var(--bg-panel)"}}>{h}</th>)}</tr></thead>
                          <tbody>
                            {cipData.map((c,i)=>(
                              <tr key={i}>
                                <td style={{padding:"8px 10px",borderBottom:"1px solid var(--border)",color:"var(--text-muted)"}}>{i+1}</td>
                                <td style={{padding:"8px 10px",borderBottom:"1px solid var(--border)"}}>
                                  <div style={{fontFamily:"monospace",fontSize:11,color:"var(--accent)"}}>{c.fileName}</div>
                                  <div style={{fontSize:10,color:"var(--text-dim)",maxWidth:220,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={c.filePath}>{c.filePath}</div>
                                </td>
                                <td style={{padding:"8px 10px",borderBottom:"1px solid var(--border)",color:"#8AACCC",fontSize:11}}>{c.types.split(";")[0]}</td>
                                <td style={{padding:"8px 10px",borderBottom:"1px solid var(--border)"}}><span style={methBadge(c.action==="MODIFICAR"?"pipeline":"manual")}>{c.action}</span></td>
                                <td style={{padding:"8px 10px",borderBottom:"1px solid var(--border)",textAlign:"center",fontWeight:700,color:"#FF4560"}}>{c.count}</td>
                                <td style={{padding:"8px 10px",borderBottom:"1px solid var(--border)",textAlign:"center",color:"#8AACCC"}}>{c.hrs}</td>
                                <td style={{padding:"8px 10px",borderBottom:"1px solid var(--border)"}}><span style={sevBadge(c.severity)}>{SEV[c.severity]?.label}</span></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
